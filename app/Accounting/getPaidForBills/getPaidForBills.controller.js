(function() {
    ...
    angular
        .module('accounting')
        .controller('GetPaidForBillsForSalesController', GetPaidForBillsForSalesController);

    /* @ngInject */
    function GetPaidForBillsForSalesController($mdToast,$mdDialog,UserService, triLoaderService,$http) {
        var vm = this;
        vm.logedUser = UserService.getCurrentUser();
          //start get sales Bills
        
          $http.get("http://35.246.143.96:3111/getAllSalesBills").then(function (response) {			   
            vm.selectedBillItem = null;
            vm.searchBillText = null;
            vm.queryBills = queryBills;
            vm.salesBills =response.data ;
            vm.selectedBill = [];
            vm.isLoading = false; 
            function queryBills($query) {
                var lowercaseQuery = angular.lowercase($query);
                return vm.salesBills.filter(function(bills) {
                    var lowercaseName = angular.lowercase(bills.Product_OutGoing_Bill_Number);
    ...
    //end
        ...
        vm.SubmitData=function(form){
            ...
            if(Cash_Payment.Amount_Of_Paying){
                ...
                vm.getPaidData.Amount_Of_Paying = Cash_Payment.Amount_Of_Paying ;
            }
                ...
                vm.getPaidData.Amount_Of_Paying = Cheque_Payment.Amount_Of_Paying ;
            }
            //we need to get only bills numbers inside an array called vm.getPaidData.BillsNumbers
            vm.getPaidData.BillsNumbers =[]
            angular.forEach(vm.selectedBill, function(item, index) {
                vm.getPaidData.BillsNumbers.push(item.Product_OutGoing_Bill_Number) ;
            });
            vm.getPaidData.Receivable_CreatedByUser = vm.logedUser.mongoID;

                $http({
                    method:"POST",
                    url:"http://35.246.143.96:3111/addReceivablesForOneBillOrMore",//35.246.143.96:3111
