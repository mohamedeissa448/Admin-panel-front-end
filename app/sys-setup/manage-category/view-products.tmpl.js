...
(function () {
    ...
    angular
        .module('sys-setup')
        .controller('ViewProductsController', ViewProductsController);

    /* @ngInject */
    function ViewProductsController($mdToast, $mdDialog,$scope, triLoaderService,UserService, $http, itemToViewItsProducts) {
        var vm = this;
      ...
      vm.products = [];
        ...
        $http({
          method: "post",
          url: "http://35.246.143.96:3111/product/getCategoriedProducts",//35.246.143.96:3111
      ...
      vm.DeleteProduct = function(product){
        console.log("productTOdelete",product)
          ...
          $http({
            method:"POST",
            url:'http://35.246.143.96:3111/deleteProductCategoryID',
            data :data
        }).then(function(data){ 
            vm.isLoading = false;
      ...
      vm.SaveProduct = function(){
          ...
          $http({
              method:"POST",
              url:'http://35.246.143.96:3111/editProductCategoryIDs',
              data :data
          }).then(function(data){ 
              vm.isLoading = false;
              if (data.data.message==true) {
                showAddToast("Product's category Updated successfully",$mdToast);
                triLoaderService.setLoaderActive(false);
            }else{
                showAddErrorToast("Something Went wrong,Please try again later!",$mdToast);
                triLoaderService.setLoaderActive(false);
