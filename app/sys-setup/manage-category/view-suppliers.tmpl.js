...
(function () {
    ...
    /* @ngInject */
    function ViewSupplierController($mdToast, $mdDialog,$scope, triLoaderService,UserService, $http, itemToViewItsSuppliers) {
      var vm = this;
      ...
      vm.suppliers = [];
        ...
        $http({
          method: "post",
          url: "http://35.246.143.96:3111/supplier/getCategoriedSuppliers",//35.246.143.96:3111
      ...
      vm.DeleteSupplier = function(supplier){
        console.log("supplierTOdelete",supplier)
          ...
          $http({
            method:"POST",
            url:'http://35.246.143.96:3111/deleteSupplierCategoryID',
            data :data
        }).then(function(data){ 
            vm.isLoading = false;
      ...
      vm.SaveSupplier = function(){
          ...
          $http({
              method:"POST",
              url:'http://35.246.143.96:3111/editSupplierCategoryIDs',
              data :data
          }).then(function(data){ 
              vm.isLoading = false;
              if (data.data.message==true) {
                showAddToast("Supplier's category Updated successfully",$mdToast);
                triLoaderService.setLoaderActive(false);
            }else{
                showAddErrorToast("Something Went wrong,Please try again later!",$mdToast);
                triLoaderService.setLoaderActive(false);
