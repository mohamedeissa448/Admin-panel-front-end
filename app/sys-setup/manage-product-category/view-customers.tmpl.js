...
(function () {
    ...
    angular
        .module('sys-setup')
        .controller('ViewCustomerControllerForProductCategory', ViewCustomerControllerForProductCategory);

    /* @ngInject */
    function ViewCustomerControllerForProductCategory($mdToast, $mdDialog,$scope, triLoaderService,UserService, $http, itemToViewItsCustomers) {
        console.log("here")
        var vm = this;
        console.log("here2")
      ...
      vm.customers = [];
        ...
        $http({
          method: "post",
          url: "http://35.246.143.96:3111/customer/getProductCategoriedCustomers",//35.246.143.96:3111
      ...
      vm.DeleteCustomer = function(customer){
        console.log("customerTOdelete",customer)
          ...
          $http({
            method:"POST",
            url:'http://35.246.143.96:3111/deleteCustomerProductCategoryID',
            data :data
        }).then(function(data){ 
            vm.isLoading = false;
      ...
      vm.SaveCustomer = function(){
          ...
          $http({
              method:"POST",
              url:'http://35.246.143.96:3111/editCustomerProductCategoryIDs',
              data :data
          }).then(function(data){ 
              vm.isLoading = false;
              if (data.data.message==true) {
                showAddToast("Customer's product category Updated successfully",$mdToast);
                triLoaderService.setLoaderActive(false);
            }else{
                showAddErrorToast("Something Went wrong,Please try again later!",$mdToast);
                triLoaderService.setLoaderActive(false);
