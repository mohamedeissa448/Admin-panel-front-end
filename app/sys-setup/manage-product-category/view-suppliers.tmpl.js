...
(function () {
    ...
    angular
        .module('sys-setup')
        .controller('ViewSupplierControllerForProductCategory', ViewSupplierControllerForProductCategory);

    /* @ngInject */
    function ViewSupplierControllerForProductCategory($mdToast, $mdDialog,$scope, triLoaderService,UserService, $http, itemToViewItsSuppliers) {
      var vm = this;
      vm.logedUser = UserService.getCurrentUser();
      vm.SendOrder = {};
      console.log("ItemToEdit",itemToViewItsSuppliers)
      vm.Category_Name=itemToViewItsSuppliers.Category_Name;
      vm.Category_ID=itemToViewItsSuppliers.Category_ID;
      ...
      vm.suppliers = [];
        ...
        $http({
          method: "post",
          url: "http://35.246.143.96:3111/supplier/getProductCategoriedSuppliers",//35.246.143.96:3111
      ...
      vm.DeleteSupplier = function(supplier){
        console.log("supplierTOdelete",supplier)
          ...
          $http({
            method:"POST",
            url:'http://35.246.143.96:3111/deleteSupplierProductCategoryID',
            data :data
        }).then(function(data){ 
            vm.isLoading = false;
      ...
      vm.SaveSupplier = function(){
          ...
          $http({
              method:"POST",
              url:'http://35.246.143.96:3111/editSupplierProductCategoryIDs',
              data :data
          }).then(function(data){ 
              vm.isLoading = false;
              if (data.data.message==true) {
                showAddToast("Supplier's product category Updated successfully",$mdToast);
                triLoaderService.setLoaderActive(false);
            }else{
                showAddErrorToast("Something Went wrong,Please try again later!",$mdToast);
                triLoaderService.setLoaderActive(false);
