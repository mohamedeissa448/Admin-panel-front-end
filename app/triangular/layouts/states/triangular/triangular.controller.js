...
(function() {
    ...
    /* @ngInject */
    function TriangularStateController($scope, $rootScope, $translate,$timeout, $templateRequest, $compile, $element, $window, triLayout, triLoaderService) {
        if(angular.isDefined(sessionStorage.currentLangDirection)){
            ...
            MainContainer.addClass('nono'+ sessionStorage.currentLangDirection);
            MainContainer.css('direction', sessionStorage.currentLangDirection);
            $translate.use(sessionStorage.currentLanguage);
        }
        else{
            var MainContainer = angular.element( document.querySelector( '#nonoMainContainer' ) );
            MainContainer = MainContainer.parent();
            MainContainer.removeClass('nonoltr');
            MainContainer.removeClass('nonortl');
            MainContainer.css('direction','ltr');
            $translate.use('en')
        }
        var loadingQueue = [];
        var vm = this;
